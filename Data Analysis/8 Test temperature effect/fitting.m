...
%   Statistically testing if W_Prun is different among the groups



function params = fitting(subjectsFlags,permutation , group)
...
%    mainFig      = figure('Position', [700, 10  , 1000,800 ]); 
    ...
    for subject = 1 : subjectsNum
        if subjectsFlags(subject)
            data{subject} = dlmread([int2str(subject   ),'.txt']);   
        else
            data{subject} = dlmread([int2str(subject+15),'.txt']);   
        end
...
%iBIC = computingIBIC (iBICSample,MU,sigma2,subjectsNum,trialsNum,freeParametersNum,iBICSamplesNum,data,paramMap,model,isParamAvailable)
    ...
    for iteration = 1 : EMiterationNum
        disp([' ----- Permutation ', int2str(permutation),' ----- Group ', int2str(group),' ----- Iteration ', int2str(iteration)]);
...
%            disp([' ----- Permutation ', int2str(permutation),' ----- Group ', int2str(group),' ----- Iteration ', int2str(iteration),' ----- Subject ', int2str(subject)]);
...
%        plotWeights                 (weightsFig,iteration,MU,sigma2,paramMap,model);
    ...
    if group==1 
        save('Results_mean1.mat','MU');
        save('Results_variance1.mat','sigma2');
        save('Results_means1.mat','w');
        save('Results_variances1.mat','s');    
    else
        save('Results_mean2.mat','MU');
        save('Results_variance2.mat','sigma2');
        save('Results_means2.mat','w');
        save('Results_variances2.mat','s');    
    end

...
%##              W_MB                       [0,1] : Model-based weight = 1-pruning weight
...
%##                                          
%##########################################################################
function behavior = agent(trialsNum,explorationRate,explorationRateLevel2,stayBias,learningRateMF,driftMF,learningRateRewardMB,learningRateTransitionMB, W_MB , W2_MB ,hoppingTrialsMean,hoppingTrialsVariance);
    ...
    for trial = 1 : trialsNum        
        ...
        values(2) = microAgentEvaluationFirstLevel ('model-based','transfer'   ,0);    

        value.action1 = values(1).action1 * W_MB +  values(2).action1 * (1-W_MB);
        value.action2 = values(1).action2 * W_MB +  values(2).action2 * (1-W_MB);
        ...
        value.action1 = values(1).action1 * W2_MB + values(2).action1 * (1-W2_MB) ;
        value.action2 = values(1).action2 * W2_MB + values(2).action2 * (1-W2_MB) ;

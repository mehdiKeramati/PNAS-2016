...
%    trialsNum = 350;
    ...
    plot(X,M(1:trialsNum,18),'b',X, X / 4,'r', 'linewidth', 2);
...
for subject = 1 : subjectsNum
    if ((totalReward(subject)/trialNum(subject)) > criteria) && ((totalReward(subject)/trialNum(subject)) < criteriaUp)
        if (subject>=fromSubject) && (subject<=toSubject)
            ...
            stayProbsRGoods(goodCounter,:) = stayProbsN(subject,:);
        end
    end
end
...
%[h,p,ci,stats] = ttest2( stayProbsRGoods(:,1)+stayProbsRGoods(:,2) , stayProbsRGoods(:,3)+stayProbsRGoods(:,4) , 'Tail','left');
...
%[h,p,ci,stats] = ttest2( stayProbsRGoods(:,1)+stayProbsRGoods(:,4) , stayProbsRGoods(:,2)+stayProbsRGoods(:,3) , 'Tail','left');
...
[h,p,ci,stats] = ttest2( stayProbsRGoods(:,1) , stayProbsRGoods(:,2) , 'Tail','left');
sprintf('For CC - CR > 0           :        p-value = %.6f',p)
%disp(transpose(stayProbsRGoods(:,1) - stayProbsRGoods(:,2)))
disp('---------------------------------------------')

[h,p,ci,stats] = ttest2( stayProbsRGoods(:,1) , stayProbsRGoods(:,3) , 'Tail','left');
sprintf('For CC - RC > 0           :        p-value = %.6f',p) 
%disp(transpose(stayProbsRGoods(:,1) - stayProbsRGoods(:,3)))
disp('---------------------------------------------')

[h,p,ci,stats] = ttest2( stayProbsRGoods(:,2) , stayProbsRGoods(:,3) , 'Tail','left');
...
axis([0.5 ,2.5,-0.05,1.05])
%axis([0.5,2.5,0.75+(group-2)*0.1 , 1.85-(group-2)*0.2])
xlabel('Previous transition');
ylabel('average stay propability') 
ax = gca;
ax.TitleFontSizeMultiplier =.9;
